...
def test_environment_detection():
    ...
    for i, case in enumerate(test_cases, 1):
        ...
        if (
            case["origin"]
            and "localhost:2718" in case["origin"]
            and "?file=" in case["href"]
        ):
...
def test_state_generation_cases():
    ...
    for i, case in enumerate(test_cases, 1):
        ...
        if (
            case["href"]
            and "localhost:2718" in case["href"]
            and "?file=" in case["href"]
        ):
            print("   ✅ Workspace mode detected, preserving file parameter")
            state_href = case["href"]
        elif case["href"] and "localhost:2718" in case["href"]:
            print("   ℹ️  Localhost:2718 detected but no file parameter")
            state_href = case["href"]
        elif case["href"] and (
            "login" in case["href"] or "pkceflow_login" in case["href"]
        ):
            # Simulate WASM environment logic
            parsed = urlparse(case["href"])
            state_href = f"{parsed.scheme}://{parsed.netloc}/"
            print("   🔄 Login page detected, redirecting to main page")
        else:
            print("   ℹ️  Using original href")
        ...
        if case["expected_preserved"]:
            if state_href == case["href"]:
                print("   ✅ URL preserved correctly")
            else:
                print("   ❌ URL not preserved correctly")
        else:
            if state_href != case["href"]:
                print("   ✅ URL redirected correctly")
            else:
                print("   ❌ URL should have been redirected")
...
def test_callback_handler_cases():
    ...
    for i, case in enumerate(test_cases, 1):
        ...
        is_workspace_mode = (
            href_url.hostname == "localhost"
            and href_url.port == "2718"
            and href_url.search_params.has("file")
        )

        print(f"   Detected workspace mode: {is_workspace_mode}")

        if is_workspace_mode:
            print("   ✅ Workspace mode detected, will preserve file parameter")
        else:
            print("   ℹ️  Standard mode, will use normal redirect logic")
...
def test_url_parsing_edge_cases():
    ...
    for i, url in enumerate(edge_cases, 1):
        ...
        is_workspace_mode = (
            parsed.hostname == "localhost" and parsed.port == "2718" and has_file_param
        )
...
if __name__ == "__main__":
    ...
    print(
        "This covers all the environment detection scenarios in _configure_cloudflare_urls()"
    )
