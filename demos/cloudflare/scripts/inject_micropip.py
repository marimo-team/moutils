...
def inject_micropip_install(notebook_path):
    ...
    for code_line in micropip_code.split("\n"):
        micropip_lines.append(f"    {code_line.rstrip()}")
    ...
    while i < len(lines):
        ...
        if "from moutils." in line or "import moutils" in line:
            ...
            cell_start = i
            while cell_start > 0 and not lines[cell_start].strip().startswith(
                "@app.cell"
            ):
                cell_start -= 1
            ...
            if cell_start > 0:
                ...
                while func_line < len(lines) and not lines[
                    func_line
                ].strip().startswith("def _("):
                    func_line += 1
                ...
                if func_line < len(lines):
                    ...
                    if func_def.startswith("def _():"):
                        ...
                        lines[func_line] = func_def.replace(
                            "def _():", "def _(moutils_installed):"
                        )
                    elif (
                        func_def.startswith("def _(")
                        and "moutils_installed" not in func_def
                    ):
                        ...
                        new_func_def = (
                            func_def[:param_start] + new_params + func_def[param_end:]
                        )
    ...
    for text_line in lines:
        if text_line.strip() == "":
            ...
            blank_count = 0
            cleaned_lines.append(text_line.rstrip())
